use crate::game::world::{AmDat, EqCrt, Gmd, Itm, SklDat, SklPtDat};
use binrw::BinRead;
...
fn main() {
    ...
    let item_data_path = "common/item/itemData.itm";
    let item_eng_path = "common/text/steam/item_eng.gmd";

    let skill_data_path = "common/equip/skill_data.skl_dat";
    let skill_point_data_path = "common/equip/skill_point_data.skl_pt_dat";

    let armor_data_path = "common/equip/armor.am_dat";
    let armor_craft_path = "common/equip/armor.eq_crt";
    let armor_eng_path = "common/text/steam/armor_eng.gmd";

    let mut r;

    r = BufReader::new(File::open(unpacked.join(item_data_path)).unwrap());
    ...
    r = BufReader::new(File::open(unpacked.join(skill_point_data_path)).unwrap());
    ...
    r = BufReader::new(File::open(unpacked.join(skill_data_path)).unwrap());
    ...
    r = BufReader::new(File::open(unpacked.join(armor_data_path)).unwrap());
    ...
    r = BufReader::new(File::open(unpacked.join(armor_craft_path)).unwrap());
    let armor_craft = EqCrt::read(&mut r).expect("read error");
    fs::write(
        "./dump/armor_craft.json",
        serde_json::to_string_pretty(&armor_craft).unwrap(),
    )
    .expect("write error");
    println!("armor_craft: {}", armor_craft.entries.len());
